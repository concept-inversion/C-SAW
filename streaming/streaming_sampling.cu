...
__device__ 
void d_display(int *a,int *b, int length)
{
	for(int i=0;i<length;i++)
	{
...
__device__
int bitmap_binary_search(int start,int end,float value, float *arr, int *bitmap, int bitmap_start, int& is_in)
{
    //printf("low:%d,high:%d,value:%f\n",start,end,value); 
    int low=start;
    int high=end;
...
__global__ void
check(int Graph_block_size,int streamid,int block_id,vertex_t* adj_list, index_t* beg_pos,weight_t* weight_list,int vertex_count,curandState *global_state,int *g_node_list,int *g_edge_list,int *neigh_l,float *degree_l,int n_blocks,int *d_seed,int n_threads, int *total, int *hashtable, int *bitmap,int total_subgraphs, int *node, int *queue, int *sample_id, int *depth_tracker, int *qstart_global, int *qstop_global, int *g_sub_index)
{
	...
	//-----------------We may require a barrier here for storing temp queue---------------------//
	...
	{
		if(warp_tid==0) {q_start= atomicAdd(&qstart_global[block_id],1);}
		...
		while(q_start<qstop_global[block_id])
		{
			...
			if(prefix)
			{
				...
				for(int i=warp_tid+neighbor_start; i<neighbor_end;i+=warpsize)
				{
					// neighbor ID
					int temp = adj_list[i];
					// if((temp>Graph_block_size)& (warp_tid==0)){printf("Reading from outside.\n");}
					// degree of neighbor
					degree_l[index]= float(beg_pos[temp+1] - beg_pos[temp]);
					//printf("%d has a degree of %f found by %d,index:%d\n",temp,degree_l[index],threadIdx.x,index);
					index+= warpsize;
				}
				...
				// Insert 0 padding
				//if(blockIdx.x==252){printf("L:%d, bits:%f, raise:%d\n",neighbor_length,bits,raise);	}
				int len=max_bit;
				int total_step= log2f(max_bit);
				gpu_prefix(total_step,warp_tid,degree_l,offset_d_n,warpsize,len);
				//stop_time = clock();
				//prefix_time = float(stop_time-start_time);
				...
				{	
					...
					//------------------------------------Using bitmaps----------------------------------------------
					selected= bitmap_binary_search(i_start_neigh,i_end_neigh,r,degree_l,bitmap,bitmap_start,is_in);
					new_neighbor= adj_list[selected+neighbor_start- offset_d_n];
					// if(is_in==0) {printf("Index: %d, New N: %d, Thread: %d\n",selected,new_neighbor,threadIdx.x);}
			...
			{
					//if((warp_tid+neighbor_start)>234370180){printf("err: Vertex: %d, neigh_start: %d, warptid:%d\n",vertex,neighbor_start,warp_tid);};
					if(thread_flag){
						new_neighbor= adj_list[warp_tid+neighbor_start]; //unwanted thread also may get some child but will be neglected in next section
					}
			...
			if(is_in==0 && thread_flag)
			{
				...
				is_in=linear_search(new_neighbor,hashtable,bin_count,bin,BIN_OFFSET,BIN_START,BUCKETS);
			...
			{	
				...
				//{printf("Add to hashtable: vertex:%d at address:%d, bin:%d\n\n",new_neighbor,(index*BUCKETS+ bin + BIN_START),bin);}
				//-------------------------------------------------------------------------

				int g_sub_start=sample_id[q_start]*1500;
				...
				// printf("Added to sample: %d, index:%d, depth:%d, sample_id: %d\n",new_neighbor,g_to,depth_tracker[q_start],sample_id[q_start]);
				...
				if(depth_tracker[q_start] < depth_limit)
				{
					...
					//
					queue[to+queue_start_address]=new_neighbor;
					sample_id[to+queue_start_address]=sample_id[q_start+queue_start_address];
					depth_tracker[to+queue_start_address]=depth_tracker[q_start+queue_start_address]+1;
				}
			}
			//q_start+=1;
			if( (qstart_global[block_id]>qstop_global[block_id])) {break;} 
			if(warp_tid==0)
	...
	// if((threadIdx.x==0)&(blockIdx.x==0)) {printf("Going out: Stream:%d, Block:%d, Queue stop:%d, Queue start:%d \n",streamid,block_id,qstop_global[block_id],qstart_global[block_id]);}
	//qstart_global[block_id]=temp_queue_stop; // This line may require some barrier. Or we could use different variable for qstart  
	
}

int build_histogram(int n_subgraph, int *input, int *frequency,int block_window_size, int block_size, int vert_count, int vertex_block_count)
{
	int max_index=0,max_value=0;
	for(int i=0;i<n_subgraph;i++)
	{
		int block= input[i]/block_size;
		if(block>vertex_block_count){block=vertex_block_count;}
...
int block_augument(int blocks, int vertex_count,index_t* beg_pos,int *beg_size_list, int *adj_size_list)
{
...
struct arguments Sampler(char beg[100], char csr[100],int n_blocks, int n_threads, int n_subgraph, int frontier_size, int neighbor_size, struct arguments args, int rank)
{
	...
	*ginst = new graph
	...
	for(int i=0;i<4;i++)
	{
		printf("%d,%d\n",i,ginst->beg_pos[(i+1)*Graph_block_size]-ginst->beg_pos[(i)*Graph_block_size]);
	}
	curandState *d_state;
	...
	int *hashtable,*bitmap,*node,*queue,*qstop_global,*qstart_global,*sample_id,*depth_tracker,*g_sub_index,*degree_l,*prefix_status;
	// Size of blocks
	...
	for(int n=0;n<n_subgraph;n++)
    {
		...
		if(bin_new>Graph_block){bin_new=Graph_block;}
	...
	block_augument(Graph_block, vertex_count, ginst->beg_pos,beg_size_list,adj_size_list);
	...
	while(sampling_complete==false)
	{
		...
		if(block_active[1]){
			...
			H_ERR(cudaMemcpyAsync(&ggraph.beg_pos[beg_size_list[block_id2]],&ginst->beg_pos[beg_size_list[block_id2]],beg_size_list[block_id3]-beg_size_list[block_id2]
				, cudaMemcpyHostToDevice,stream2));	
		check<<<n_blocks,n_threads,1,stream2>>>(Graph_block_size,1,block_id2,ggraph.adj_list, ggraph.beg_pos, ggraph.weight_list, ggraph.vert_count,d_state,d_node_list,d_edge_list,d_neigh_l,d_degree_l,n_blocks,d_seed,n_threads,d_total,hashtable,bitmap,n_subgraph,node,queue,sample_id,depth_tracker,qstart_global,qstop_global,g_sub_index);
		}

		if(block_active[2]){
			H_ERR(cudaMemcpyAsync(&ggraph.adj_list[adj_size_list[block_id3]],&ginst->adj_list[adj_size_list[block_id3]],adj_size_list[block_id4]-adj_size_list[block_id3]
				, cudaMemcpyHostToDevice,stream3));
			H_ERR(cudaMemcpyAsync(&ggraph.beg_pos[beg_size_list[block_id3]],&ginst->beg_pos[beg_size_list[block_id3]],beg_size_list[block_id4]-beg_size_list[block_id3]
				, cudaMemcpyHostToDevice,stream3));
		check<<<n_blocks,n_threads,2,stream3>>>(Graph_block_size,2,block_id3,ggraph.adj_list, ggraph.beg_pos, ggraph.weight_list, ggraph.vert_count,d_state,d_node_list,d_edge_list,d_neigh_l,d_degree_l,n_blocks,d_seed,n_threads,d_total,hashtable,bitmap,n_subgraph,node,queue,sample_id,depth_tracker,qstart_global,qstop_global,g_sub_index);	
		}

		if(block_active[3]){
			H_ERR(cudaMemcpyAsync(&ggraph.adj_list[adj_size_list[block_id4]],&ginst->adj_list[adj_size_list[block_id4]],adj_size_list[4]-adj_size_list[block_id4]
				, cudaMemcpyHostToDevice,stream4));
			H_ERR(cudaMemcpyAsync(&ggraph.beg_pos[beg_size_list[block_id4]],&ginst->beg_pos[beg_size_list[block_id4]],beg_size_list[4]-beg_size_list[block_id4]
				, cudaMemcpyHostToDevice,stream4));
		check<<<n_blocks,n_threads,3,stream4>>>(Graph_block_size,3,block_id4,ggraph.adj_list, ggraph.beg_pos, ggraph.weight_list, ggraph.vert_count,d_state,d_node_list,d_edge_list,d_neigh_l,d_degree_l,n_blocks,d_seed,n_threads,d_total,hashtable,bitmap,n_subgraph,node,queue,sample_id,depth_tracker,qstart_global,qstop_global,g_sub_index);
		}
		//wait for completion
		...
		HRR(cudaMemcpy(start_queue,qstart_global,sizeof(int)*Graph_block, cudaMemcpyDeviceToHost));
		HRR(cudaMemcpy(seeds_counter,qstop_global,sizeof(int)*Graph_block, cudaMemcpyDeviceToHost));
		max=0,value=0;
		q_count=0;
		...
		if(i==10){
			//printf("Sampling complete;\n");
			sampling_complete=true;}
		// printf("Total: %d,max:%d, value:%d, Value of i: %d\n",q_count,max,value,i);
	}
	HRR(cudaDeviceSynchronize());
